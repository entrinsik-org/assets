    ...
    _deployResource(server, resource) {
        resource.paths.forEach(path => {
            ...
            server.route({
                method: 'GET',
                path: path.url,
                config: _.defaults({ auth: false, handler: { file: path.file } }, this.options.routeConfig)
            });
        });
    }

    deployTo(server) {
        server.ext('onPreStart', (s, d) => this.deploy(s, this.options).nodeify(d));
        return this;
    }
    ...
    deploy(server) {
        return this.compile(this.options)
            .then(resources => {
                ...
                _.filter(resources, { autoroute: true }).forEach(resource => this._deployResource(server, resource));
            });
    }
